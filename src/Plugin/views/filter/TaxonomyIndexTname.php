...
namespace Drupal\custom_tables\Plugin\views\filter;
 ...
 *
 * @ViewsFilter("taxonomy_index_tname")
 */
class TaxonomyIndexTname extends TaxonomyIndexTid {
  ...
  /**
   * Constructs a TaxonomyIndexTname object.
   *
  ...
  public function buildExtraOptionsForm(&$form, FormStateInterface $form_state) {
    ...
    foreach ($vocabularies as $voc) {
      ...
      //drupal_set_message(json_encode($voc->id()."__".$voc->label()." "));
    }
      ...
      if (!empty($this->options['hierarchy']) && $this->options['limit']) {
        ...
        if ($tree) {
          foreach ($tree as $term) {
            $choice = new \stdClass();
            $choice->option = [
              $term->id() => str_repeat('-', $term->depth) . \Drupal::service('entity.repository')
                  ->getTranslationFromContext($term)
                  ->label(),
            ];
            $options[] = $choice;
        ...
        foreach ($terms as $term) {
          $options[$term->label()] = \Drupal::service('entity.repository')
            ->getTranslationFromContext($term)
            ->label();
        }
      }
